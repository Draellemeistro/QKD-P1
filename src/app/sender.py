...
from src.app.transfer.transport import Transport, encode_packet_with_headers
...
def send_chunk_packet(transport, chunk, key_data):
    """
    Encrypts a single chunk, formats the packet with headers, and sends it.
    """
    # 1. Encrypt Payload
    ...
    headers = {
        ...
        "key_block_id": str(key_data["blockId"]),
        "key_index": str(key_data["index"]),
        "is_last": "false"
    }

    # 3. Encode with headers & Send
    packet_bytes = encode_packet_with_headers(headers, encrypted_payload.encode('utf-8'))
    transport.send_reliable(packet_bytes)
...
def run_file_transfer(receiver_id, destination_ip, destination_port, file_path):
    ...
    end_packet = encode_packet_with_headers(end_headers, b"")
