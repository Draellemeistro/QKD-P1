...
from src.app.transfer.transport import Transport, decode_packet_with_headers
    ...
    with FileStreamWriter(output_file) as writer:
        ...
        while True:
            ...
            if event.type == enet.EVENT_TYPE_RECEIVE:
                try:
                    # 2. Parse Protocol (headers + data)
                    packet_data = event.packet.data
                    headers, encrypted_data = decode_packet_with_headers(packet_data)
                    
                    # Convert headers to dict format expected by process_single_packet
                    packet_dict = {
